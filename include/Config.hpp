...
#define SOUND_VALUE(name, folder, ...) VALUE_DEFAULT(Sound, name, Sound(soundPath + folder + "/", soundPath + folder + "/" + #name + ".ogg", __VA_ARGS__))
#define SOUND_VALUE_SIMPLE(name, folder) VALUE_DEFAULT(Sound, name, Sound(soundPath + folder + "/", soundPath + folder + "/" + #name + ".ogg"))
...
namespace QuestSounds {
    ...
    DECLARE_JSON_CLASS(Sound,
        ...
        Sound(std::string folderPath, std::string defaultFilePath, std::optional<float> volumeOffset = std::nullopt, std::optional<float> beatOffset = std::nullopt) : FolderPath(folderPath), DefaultFilePath(defaultFilePath), VolumeOffset(volumeOffset), BeatOffset(beatOffset) {}
    )
